...
var _ = Describe("AppsMetrics", func() {
	Context("app-metrics command", func() {
		It("prints usage if application name isn't provided", func() {
	...
	Context("app-metrics-prometheus command", func() {
		It("prints usage if application name isn't provided", func() {
			...
			output := CaptureOutput(func() {
				appsMetricsPlugin.Run(fakeCliConnection, []string{"app-metrics-prometheus"})
			})

			Expect(output).To(ContainElement("cf app-metrics-prometheus APP_NAME"))
		})
		...
		It("prints error for unknown app", func() {
			...
			output := CaptureOutput(func() {
				appsMetricsPlugin.Run(fakeCliConnection, []string{"app-metrics-prometheus", "iDoNotExist"})
			})
		...
		It("prints error when unrecognized flag is set", func() {
			...
			output := CaptureOutput(func() {
				plugin.Run(fakeCliConnection, []string{"app-metrics-prometheus", "some-app", "-unknownFlag"})
			})
		...
		It("prints error when unable to get metrics", func() {
			...
			output := CaptureOutput(func() {
				plugin.Run(fakeCliConnection, []string{"app-metrics-prometheus", "some-app"})
			})
